...
#define WindowHeight 480
	...
	if (!hMainWindow) return FALSE;
	{
		...
		for (int i=0; i<sizeof(regPaths) / sizeof(regPaths[0]);++i) {
			...
			if (FirstBoot) MessageBox(NULL,"Dear User,\n\n\nI am no longer maintaining or developing this 1.6.2 build anymore due to my circumstances. As such, I suggest to either use another N64 emulator, or modify this one if you experience any issue while using it. Do note that there's a whole guide document written up explaining various things about the application which can be opened from the Help tab, consult this document for information about the application and its plugins. Consider swapping to plugins that are supported by your system and work the best for the game you're trying to play and refer to any Plugin Notes from the ROM Notes tab under Options->Settings if they are provided for a registered game.\n\n\nFor contact, my email is bruceiv.shankle@gmail.com.\n\n\n\n- Bruce Shankle IV",AppName,MB_OK|MB_ICONINFORMATION|MB_SETFOREGROUND);
			FirstBoot=FALSE;
			}
		}
	}
